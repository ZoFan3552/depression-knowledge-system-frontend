...
const UserAvatar: React.FC<UserAvatarProps> = ({ username }) => {
  ...
  const handleLogout = async (): Promise<void> => {
    try {
      ...
      console.log(error);
      // 捕获并处理可能的异常
      showToast("登出时发生错误", 3000, "error", () => router.push("/login"));
    }
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-800 text-white transition-colors duration-200 hover:bg-blue-700 focus:outline-none"
        aria-label="用户菜单"
        title={username}
      >
        {userInitial}
