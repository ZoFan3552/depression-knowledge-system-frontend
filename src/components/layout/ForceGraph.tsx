...
const ForceGraph: React.FC<ForceGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const linkLabel =  g.selectAll('.link-label')
      ...
      .text(d => d.text || "none");    // 显示文本内容，默认为"none"
...
}, [nodes, links, width, height, graphConfig, handleDrag, initializeSimulation, nodeRadius, zoomLevel, createArrowMarker]);
...
const resetZoom = useCallback(() => {
  ...
  svg.transition()
    ...
    .call(zoomRef.current.transform, d3.zoomIdentity
      .translate(width / 2, height / 2) // 初始平移
      .scale(1)                        // 重置为原始比例
      .translate(-width / 2, -height / 2)); // 反向平移

  setZoomLevel(1); // 重置缩放级别状态
}, [width, height]);
