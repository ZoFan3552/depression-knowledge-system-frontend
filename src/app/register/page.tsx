...
const RegisterPage = () => {
  ...
  // 初始化待注册用户数据状态
  const [, setRegisterUser] = useState<User>({
    username: "",
    email: "",
    password: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 捕获并处理异常
      setError(
        "注册失败：" + (err instanceof Error ? err.message : "未知错误"),
      );
    } finally {
      // 无论成功失败，重置提交状态
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-blue-50 via-white to-blue-50 px-4">
      <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-8 shadow-lg">
        ...
        <div className="text-center">
          <h2 className="mb-2 text-3xl font-bold text-blue-800">注册账号</h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="group relative flex w-full transform justify-center rounded-lg border border-transparent bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-all duration-150 hover:scale-[1.02] hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
          >
            注册
          </button>
          {/* 返回登录 */}
